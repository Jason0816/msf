...
export default function MihomoProxiesPage() {
  ...
  const groups = useMemo(() => {
    ...
    const nodeKeys = new Set(searchProxyStore(store, deferredSearch, "nodes", { regex }).results.map((item) => item.key));
    return matched.groups
      ...
      .map((group) => {
        ...
        const members = selectGroupNodes(store, groupKey, true).filter((node) => searchMode !== "nodes" || nodeKeys.has(node.key));
        ...
        return {
          ...
          trafficSpeed: groupTraffic[group.name]?.total ?? 0,
          hidden: group.hidden,
  ...
  }, [deferredSearch, groupTraffic, regex, resolveChain, searchMode, settings, store]);
  ...
  return (
    <AppShell>
      ...
      <ProxySettingsDialog open={settingsOpen} settings={settingsView} onChange={(value) => setSettings((current) => ({ ...current, ...value, sortBy: value.sortBy as ProxyPageSettings["sortBy"] }))} onClose={() => setSettingsOpen(false)} onReset={() => setSettings(resetProxySettings())} />
