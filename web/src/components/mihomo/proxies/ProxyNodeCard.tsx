  ...
  return (
    ...
    >
      <div className={cn("flex min-w-0 items-start", cardSize === "compact" ? "gap-1.5" : "gap-2")}>
        ...
        {onSelect ? (
          <button type="button" onClick={onSelect} className={cn("min-w-0 flex-1 text-left text-xs font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/45", display === "wrap" ? "break-words" : "truncate")} title={`选择 ${node.name}`}>
            {node.name}
          </button>
