...
export function FaviconLatencyTester() {
  ...
  return <SolidPlate className="flex flex-col rounded-xl p-4">
...
export function ConnectionSankey({ connections }: { connections: OverviewConnection[] }) {
  ...
  return <GlassSurface material="thick" className={cn("rounded-2xl p-4", fullScreen && "fixed inset-0 z-[120] rounded-none bg-background p-4")}>
    ...
    <div className={cn("relative mt-4 h-96 w-full overflow-hidden rounded-xl bg-background/35", fullScreen && "h-[calc(100vh-3rem)] rounded-none")}>
...
export function ConnectionHistoryPanel({ connections }: { connections: OverviewConnection[] }) {
  ...
  return <GlassSurface material="thick" className="rounded-2xl p-4">{sectionTitle(<Clock3 className="h-4 w-4" />, "连接统计", "仅在连接结束时写入历史，当前活跃连接只参与即时汇总", <div className="flex flex-wrap items-center gap-1.5"><select value={aggregation} onChange={(event) => { const value = event.target.value as HistoryAggregation; setAggregation(value); localStorage.setItem("msf-mihomo-history-aggregation", value); }} className="h-8 rounded-lg border border-border/55 bg-background px-2 text-xs">{Object.entries(labels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select><select value={cleanupDays} onChange={(event) => { const value = Number(event.target.value); setCleanupDays(value); localStorage.setItem("msf-mihomo-history-cleanup-days", String(value)); }} className="h-8 rounded-lg border border-border/55 bg-background px-2 text-xs"><option value={0}>永不清理</option><option value={7}>保留一周</option><option value={30}>保留一月</option><option value={90}>保留三月</option></select><button type="button" onClick={() => void clearClosedConnections().then(() => setClosed([]))} className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive" title="清空连接历史"><Trash2 className="h-3.5 w-3.5" /></button></div>)}<div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-5">{[[labels[aggregation], rows.length], ["总流量", formatBytes(totals.download + totals.upload)], ["下载", formatBytes(totals.download)], ["上传", formatBytes(totals.upload)], ["连接次数", totals.count]].map(([label, value]) => <SolidPlate key={String(label)} className="p-3"><div className="text-[10px] text-muted-foreground">{label}</div><div className="mt-1 truncate text-lg font-light tabular-nums text-foreground">{value}</div></SolidPlate>)}</div>{rows.length ? <div className="max-h-96 overflow-y-auto rounded-xl bg-background/35"><table className="w-full table-fixed text-xs"><thead className="sticky top-0 z-10 bg-card"><tr><th className="w-[36%] px-3 py-2 text-left">{labels[aggregation]}</th><th className="px-2 py-2 text-right">下载</th><th className="px-2 py-2 text-right">上传</th><th className="px-2 py-2 text-right">总量</th><th className="px-2 py-2 text-right">次数</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key} className="border-t border-border/30"><td className="truncate px-3 py-2 font-mono" title={row.key}>{row.key}</td><td className="px-2 py-2 text-right tabular-nums">{formatBytes(row.download)}</td><td className="px-2 py-2 text-right tabular-nums">{formatBytes(row.upload)}</td><td className="px-2 py-2 text-right tabular-nums">{formatBytes(row.download + row.upload)}</td><td className="px-2 py-2 text-right tabular-nums">{row.count}</td></tr>)}</tbody></table></div> : <EmptyState>等待连接结束后生成历史统计</EmptyState>}</GlassSurface>;
}
