...
export function useProxyGroupTraffic(options: UseProxyGroupTrafficOptions = {}): UseProxyGroupTrafficResult {
  ...
  const [counters, setCounters] = useState<Map<string, ProxyConnectionCounter>>(() => new Map());
  const [sampledAt, setSampledAt] = useState<number | undefined>();
  ...
  const refresh = useCallback(async (): Promise<ProxyGroupTrafficMap | undefined> => {
    ...
    try {
      ...
      setCounters(sampled.counters);
      setTraffic(sampled.traffic);
      setSampledAt(sampled.sampledAt);
  ...
  return { traffic, groupTraffic: traffic, counters, sampledAt, loading, error, visible, refresh };
}
