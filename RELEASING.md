...
最后一条必须无输出。发布版本以 `0.4.1` 这类不带 `v` 的值传给 Make，tag 使用 `v0.4.1`。
...
```text
MSF-0.4.1-macos-universal-unsigned.dmg
MSF-0.4.1-macos-universal-unsigned.dmg.sha256
MSF-0.4.1-macos-universal-unsigned.zip
MSF-0.4.1-macos-universal-unsigned.zip.sha256
```
...
确认 GitHub Release 包含全部 20 个安装资产和 SHA-256，GHCR 同时存在 `v0.4.1` 和 `latest`，且 revision 与 Release tag commit 相同。

下载线上 macOS 资产后再校验摘要：

```bash
shasum -a 256 -c MSF-0.4.1-macos-universal-unsigned.dmg.sha256
shasum -a 256 -c MSF-0.4.1-macos-universal-unsigned.zip.sha256
```
...
确认 v0.4.1 Release、DMG/ZIP、fnOS FPK 和 Docker 镜像全部可用后，删除本地与远程的：
